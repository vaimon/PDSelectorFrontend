...
import { expectNoSidewaysScroll, newcomer, seeded, signIn } from './support/people';

// The student journey against the real backend, one step building on the previous one. Steps still
// to come with their features: the join link (#24), invites (#16), "how it works" (#25) and the
// admin board (#27) each add their own step here.
//
...
async function fillQuestionnaire(page, testInfo, { course, group, contact }) {
...
test('a first-year asks to join, the lead accepts, and both sides see it', async ({ browser }, testInfo) => {
  ...
  await openMyTeamSection(lead);
  await lead.locator('.card').filter({ has: lead.getByRole('heading', { name: teamName }) })
    .getByRole('link').click();
  await expect(lead.getByRole('heading', { name: teamName, level: 1 })).toBeVisible();
